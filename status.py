...
def dist_from_data(training_set, testing_inst, status_num):
	distances = []
...
def knn(training_set, testing_inst, k, status_num):
	distances = dist_from_data(training_set, testing_inst, status_num)
...
def run():
	...
	for i in xrange(10):
		...
		testing_set = testing_set[:30]
		for test_data in testing_set:
			status_length = len(training_set[0]['statuses'])
			count_within += 1
			for j in xrange(0, status_length, 10):
				neighbors = knn(training_set, test_data, 25, j + 1)
				prediction = predict(neighbors)
				if (prediction == test_data['projects'][2]):
					status_accuracy[j] = (((status_accuracy[j] * (count_within-1)) + 1) / float(count_within))
				else:
					status_accuracy[j] = (((status_accuracy[j] * (count_within-1))) / float(count_within))
				print "status", j
			if i == 0:
				cPickle.dump(status_accuracy, open("status.p", "wb"))
		break
